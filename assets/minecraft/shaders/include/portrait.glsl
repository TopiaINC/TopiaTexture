// Raytraced player portrait, drawn inside a text glyph's quad.
//
// Adapted from cosrnic/text-portrait-shader, itself based on JNNGL's
// gui_player_models_base (Origin Realms). Ported to Minecraft 1.21.11 /
// resource pack format 75.
//
// How it works: the client binds the skin of the player named in a
// {"type":"object","object":"player"} text component as Sampler0. This file
// fires a ray per pixel of the glyph quad and intersects it against boxes laid
// out like a player model, reading the skin at the hit point. That is why it
// shows any player's real skin while still being a custom-shaped render — the
// thing no model or entity approach could manage.
//
// The whole body including legs is always drawn. How much of it you actually see
// is decided by framing, which the server sends in the colour's blue channel:
// 100 frames head and torso (the reference look), 200 pulls back to the feet.
...
intersection rayTrace(vec3 origin, vec3 direction, float far) {
    ...
    // overlay (the skin's outer layer: jacket, sleeves, hat)
...
vec4 portraitRender(vec2 uv, float aspectRatio, float gameTime) {
    ...
    // GameTime runs 0..1 over a 20-minute Minecraft day, so this is
    // 2*pi * 1200 / seconds-per-turn. 942 is roughly one turn every 8s.
    // Set to 0.0 to stand still.
    const float rotationSpeed = 942.0;
